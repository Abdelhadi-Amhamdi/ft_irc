...
/*   Updated: 2024/01/23 19:23:43 by kben-ham         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ChannelSource::broadcastMeassges(const std::string &channel_name, const std::string &message, std::string user) {
	...
	if (ch != channels.end())
		ch->second->brodCastMessage(message, user);
}

void ChannelSource::send_to_all(const std::string &channel_name, const std::string &message, std::string user) {
	std::map<std::string, Channel*>::iterator ch = channels.find(channel_name);
	if (ch != channels.end())
		ch->second->to_all(message, user);
}
